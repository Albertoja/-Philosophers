...
/*   Updated: 2022/09/26 17:01:10 by aespinos         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	all_ate(t_list *philos)
{
	...
	while (philos)
	{
		pthread_mutex_lock(&philos->data->mutex_eat);
		if (philos->nbr_ate < philos->data->n_eat)
		{
			pthread_mutex_unlock(&philos->data->mutex_eat);
			return (0);
		}
		pthread_mutex_unlock(&philos->data->mutex_eat);
		philos = philos->next;
...
static int	death_checker(t_list *philos)
{
	while (philos)
	{
		...
		if (ft_timer(philos->data->time_start) - philos->last_dinner
			> philos->data->time_die)
		{
			...
			printf(RED"[%lu ms] philo %d has died\n",
				ft_timer(philos->data->time_start), philos->philo + 1);
